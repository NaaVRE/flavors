...
COPY ./flavors/et-zeeland/install_packages.R .
RUN mamba run -n et-zeeland bash -c 'export PKG_LIBS="$(pkg-config --libs proj) $(gsl-config --libs)"; Rscript install_packages.R'
