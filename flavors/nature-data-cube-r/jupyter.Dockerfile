...
COPY --chown=${NB_UID}:${NB_GID} ./docker/jupyter.requirements.txt requirements.txt
...
ARG CONDA_ENV_FILE
COPY --chown=${NB_UID}:${NB_GID} ${CONDA_ENV_FILE?} environment.yaml
...
RUN mamba run -n nature-data-cube-r bash -c 'Rscript install_packages.R'
